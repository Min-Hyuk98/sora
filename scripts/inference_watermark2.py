...
from opensora.datasets.utils import save_image_sample
...
def main():
    ...
    if cfg.watermarked_eval:
        # 여기서 1500M정도 사용
        checkpoint_path = cfg.watermarked_vae_ckpt
        ldm_decoder_state_dict = torch.load(checkpoint_path)['ldm_decoder']
        vae.load_state_dict(ldm_decoder_state_dict)
        for layer in vae.parameters():
    ...
    save_dir = cfg.output_save_dir
    os.makedirs(save_dir, exist_ok=True)
    sample_name = cfg.get("sample_name", None)
    prompt_as_path = cfg.get("prompt_as_path", False)
    
    with open(os.path.join(cfg.output_dir, 'additional_args.pkl'), 'rb') as file:
        save_list_additional_args = pickle.load(file)
    with open(os.path.join(cfg.output_dir, 'model_args.pkl'), 'rb') as file:
        save_list_model_args = pickle.load(file)
        ...
        for k in range(num_sample):
            ...
            for loop_i in range(loop):
                ...
                samples = scheduler.sample(
                    ...
                    z=z,
                    device=device,
                    model_args=save_list_model_args[i],
                    additional_args=save_list_additional_args[0],
                    progress=verbose >= 2,
                    mask=masks,
                )
            ...
            if is_main_process():
                for idx, batch_prompt in enumerate(batch_prompts):
                    ...
                    for i in range(0, loop):
                        ...
                        save_image_path = save_image_sample(
                            video[i],
                            save_path=save_path,
                            verbose=verbose >= 2,
                        )
