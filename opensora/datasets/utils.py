...
def save_image_sample(x, save_path=None, normalize=True, value_range=(-1, 1), verbose=True):
    """
    Args:
        x (Tensor): shape [C, T, H, W]
    """
    ...
    for i in range(x.shape[1]):
        save_name = str(i) + ".png"
        save_image([x[:,i,:,:]], save_path+save_name, normalize=normalize, value_range=value_range)
