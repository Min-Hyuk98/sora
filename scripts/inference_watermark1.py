...
def main():
    ...
    save_list_additional_args = []
    save_list_model_args = []
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        save_list_additional_args.append(model_args)
        ...
        for k in range(num_sample):
            ...
            for loop_i in range(loop):
                ...
                ## text encoder
                n = len(batch_prompts_loop)
                model_args_ = text_encoder.encode(batch_prompts_loop)
                y_null = text_encoder.null(n)
                model_args_["y"] = torch.cat([model_args_["y"], y_null], 0)
                if model_args is not None:
                    model_args_.update(model_args)
                save_list_model_args.append(model_args_)
                
                    


        start_idx += len(batch_prompts)
    logger.info("save the result of text encoder.")

    with open(os.path.join(cfg.output_dir, 'additional_args.pkl'), 'wb') as file:
        pickle.dump(save_list_additional_args, file)
    with open(os.path.join(cfg.output_dir, 'model_args.pkl'), 'wb') as file:
        pickle.dump(save_list_model_args, file)
